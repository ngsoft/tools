...
use NGSOFT\Exceptions\RegExpException;
...
class RegExp implements \Stringable, \JsonSerializable
{
     ...
     * Initialize RegExp.
     *
     * @see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp Same as that except pattern is a string
     *
     * @param string          $pattern full pattern or pattern without delimiters and modifiers
        ...
        foreach (str_split(self::PCRE_DELIMITERS) as $delimiter)
        {
            if (\str_starts_with($pattern, $delimiter))
            {
                if (preg_match(sprintf('/[\%s]([a-z]*)$/i', $delimiter), $pattern, $matches) > 0)
                {
                    ...
                    if ( ! empty($addedFlags))
                    {
                        $flags = array_merge($flags, \mb_str_split($addedFlags));
                    }
                    break;
                }
     ...
     * @param string|string[] $flags   modifiers
     */
    public static function create(string $pattern, array|string $flags = ''): static
    {
     ...
     *
     * @return null|array if the match fails, the exec() method returns null, and sets lastIndex to 0
     */
     ...
     * @throws \TypeError
     */
    public function replace(string $str, callable|string|\Stringable $replacement): string
    {
        $this->assertValidRegex();

        if (
            ! is_callable($replacement)
            and ! is_string($replacement)
        ) {
